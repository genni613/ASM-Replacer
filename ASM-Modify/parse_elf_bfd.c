...
void info_abfd(bfd *abfd) {
  ...
  printf("start_address: %lx, ", abfd->start_address);
  printf("target_xvec_name: %s, ", abfd->xvec->name);
  printf("origin: %-8lx, ", abfd->origin);
  printf("proxy_origin: %-8lx, ", abfd->proxy_origin);
  printf("\n");
}

void info_asection(asection *sect) {
  printf("sect name: %-20s, ", sect->name);
  printf("size: %-8lx, ", sect->size);
  printf("filepos: %-8lx,", sect->filepos);
  // printf("file_ptr origin: %-8x, ", sect->origin);
  // printf("offset: %-8d, ", sect->offset);
  printf("vma: 0x%lx, ", sect->vma);
...
void parse_dynamic_symtab(bfd *abfd, parse_info_t *parse_info) {
  ...
  if (storage < 0) {
    ...
    bfd_fatal("%s", bfd_get_filename(abfd));
  }
  if (storage) sy = (asymbol **)malloc(storage);

  dynsymcount = bfd_canonicalize_dynamic_symtab(abfd, sy);
  if (dynsymcount < 0) bfd_fatal("%s", bfd_get_filename(abfd));
